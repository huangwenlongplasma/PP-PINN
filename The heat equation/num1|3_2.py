...
def compute_loss(model, x, t, x_data, t_data, real, x_fp, t_fp,  previous_solution):
    l = 2
    u_s = torch.sin(torch.pi * x + 2 * torch.pi * c * t)
    u_c = torch.cos(torch.pi * x + 2 * torch.pi * c * t)
    u = model(x, t)
    u_t = torch.autograd.grad(u.sum(), t, create_graph=True)[0]
    u_x = torch.autograd.grad(u.sum(), x, create_graph=True)[0]
    u_xx = torch.autograd.grad(u_x.sum(), x, create_graph=True)[0]
    f = u_t - c*u**l * u_xx*(alpha**l) - c*(l*u**(l-1)*u_x**l)*(alpha**l) +c*(alpha**l * l * u**(l-1) * (2 * torch.pi * u_c)**2\
        - alpha**l * u**l * (2 * torch.pi**2 * u_s) - 4 * torch.pi * torch.cos(2 * torch.pi * c*t + torch.pi * x))
    ...
    loss_bc1 = torch.mean((u_left - (1 + 2 * torch.sin(2 * torch.pi *c* t))) ** 2)
    loss_bc2 = torch.mean((u_right - (1 + 2 * torch.sin(2 * torch.pi * c*t + torch.pi))) ** 2)
...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/non_3_0.1_1.pth')
    # 创建随机训练数据集
    num_samples = 5000
    ...
    data = scipy.io.loadmat('data/non_pred3_0.1_1.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/non_fp3_0.1_1.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    optimizer_lbfgs = torch.optim.LBFGS(model.parameters(),  max_iter=6000, max_eval=None, tolerance_grad=0,
    ...
    uxt = model(X, T)
    real_u = alpha * real_u.cpu().detach().numpy()
    print(real_u.shape)
    print(X.shape)
    print(T.shape)
    uxt = alpha * uxt.cpu().detach().numpy()
    ...
    plt.xlabel('t')
    plt.ylabel('x')
    plt.savefig('fig/non_error_1_2.png')
    plt.show()
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    plt.savefig('fig/non_pred_0.1_3_2.png')
#     plt.savefig('fig/non_pred_1_2.pdf')
    plt.show()

    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
    surf2 = ax.plot_surface(X.cpu().detach().numpy().reshape(1000, 1000), T_scaled.cpu().detach().numpy().reshape(1000, 1000),real_u.reshape(1000, 1000), cmap='jet')
    # fig.colorbar(surf, shrink=0.5, aspect=5)
    
#     ax.set_zticks([min(real_u), (max(real_u) + min(real_u)) / 2, max(real_u)])  # z 轴分为三个刻度
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    ax.yaxis.labelpad = 20
    ax.zaxis.labelpad = 20
    plt.savefig('fig/non_real_0.1_3_2.png')
#     plt.savefig('fig/non_real_1_2.pdf')

    plt.show()

    save_path = 'model/non_1_2gy.pth'
