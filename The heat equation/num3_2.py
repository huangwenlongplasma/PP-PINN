...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/non_3_5_1.pth')
    # 创建随机训练数据集
    num_samples = 5000
    ...
    data = scipy.io.loadmat('data/non_pred3_5_1.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/non_fp3_5_1.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    optimizer_lbfgs = torch.optim.LBFGS(model.parameters(),  max_iter=50000, max_eval=None, tolerance_grad=0,
    ...
    uxt = model(X, T)
    real_u = 3*real_u.cpu().detach().numpy()
    print(real_u.shape)
    print(X.shape)
    print(T.shape)
    uxt = 3*uxt.cpu().detach().numpy()
    ...
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/non_pred3_5_2.mat',
    ...
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/non_fp3_5_2.mat',
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    plt.savefig('fig/non_pred_5_3_2.png')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    ax.yaxis.labelpad = 20
    ax.zaxis.labelpad = 20
    plt.savefig('fig/non_real_5_3_2.png')
#     plt.savefig('fig/non_real_1_2.pdf')

    plt.show()

    save_path = 'model/non_3_5_2.pth'
