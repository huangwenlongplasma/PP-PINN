...
def compute_loss(model, x, t, x_data, t_data, real, x_fp, t_fp,  previous_solution):
    ...
    f = u_t - 0.2*u**2 * u_xx - 0.2*2*u*u_x**2 + 0.2*(2 * u * (2 * torch.pi * torch.cos(2 * torch.pi * 0.2*t + torch.pi * x))**2\
        - u**2 * (2 * torch.pi**2 * torch.sin(2 * torch.pi * 0.2*t + torch.pi * x)) - 4 * torch.pi * torch.cos(2 * torch.pi * 0.2*t + torch.pi * x))
...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/non_2_1.pth')
    # 创建随机训练数据集
    num_samples = 5000
    ...
    data = scipy.io.loadmat('data/non_pred2_1.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/non_fp2_1.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    plt.xlabel('t')
    plt.ylabel('x')
    plt.savefig('fig/non_error_2_2.png')
    plt.show()
    ...
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/non_pred2_2.mat',
    ...
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/non_fp2_2.mat',
    ...
    ax.set_ylabel('t')
    ax.set_zlabel('u')
    plt.savefig('fig/non_pred_2_2.png')
    plt.savefig('fig/non_pred_2_2.pdf')
    plt.show()
    ...
    ax.set_ylabel('t')
    ax.set_zlabel('u')
    plt.savefig('fig/non_real_2_2.png')
    plt.savefig('fig/non_real_2_2.pdf')
    plt.show()

    save_path = 'model/non_2_2.pth'
