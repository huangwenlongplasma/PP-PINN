...
if __name__ == '__main__':
    ...
    plt.savefig('fig/loss_pinnnon_5.png')
    plt.show()
    # 测试模型
    model.eval()
    ...
    uxt = model(X, T)
    real_u = (1/3)*real_u.cpu().detach().numpy()
    print(real_u.shape)
    print(X.shape)
    print(T.shape)
    uxt = (1/3)*uxt.cpu().detach().numpy()
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    plt.savefig('fig/ptnon_pred_0.2_pinn+p.png')
...
#     ax.yaxis.labelpad = 20
#     ax.zaxis.labelpad = 20
#     plt.savefig('fig/pinnnon_real_0.2.png')
