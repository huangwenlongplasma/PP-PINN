...
if __name__ == '__main__':
    ...
    plt.savefig('fig/loss_pinnnon_0.2.png')
    plt.show()
    # 测试模型
    model.eval()
    ...
    L2_error1 = np.linalg.norm((real_u - uxt).flatten(), 2) / np.linalg.norm(real_u.flatten(), 2)
    print('相对误差: %e' % (L2_error1))
    Error1 = np.mean(np.abs(real_u - uxt))
    print('绝对误差: %e' % (Error1))
...
#     plt.xlabel('t')
#     plt.ylabel('x')
#     plt.savefig('fig/ptnon_error_1_1.png')
#     plt.show()

#     # 保存数据
#     x = torch.linspace(0, 1, 1000).to(device)
#     t = torch.linspace(0, 0.2, 1000).to(device)
#     x = x.unsqueeze(1)
#     t = t.unsqueeze(1)
#     solution = model(x, t)
#     print(solution.shape)
#     scipy.io.savemat('data/ptnon_pred5_1.mat',
...
#     u_fp = model(x, t_fp)
#     scipy.io.savemat('data/non_fp5_1.mat',
...
#                       'u': u_fp.cpu().detach().numpy().reshape(-1, 1)})
    ...
    ax.set_title('PC-PINN', fontsize=23)
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    plt.savefig('fig/ptnon_pred_5_pinn+p.png')
