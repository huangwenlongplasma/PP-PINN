...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/non_3_1.pth')
    # 创建随机训练数据集
    num_samples = 5000
    ...
    data = scipy.io.loadmat('data/non_pred3_1.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/non_fp3_1.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    optimizer_lbfgs = torch.optim.LBFGS(model.parameters(),  max_iter=13000, max_eval=None, tolerance_grad=0,
    ...
    plt.xlabel('t')
    plt.ylabel('x')
    plt.savefig('fig/non_error_3_2.png')
    plt.show()
    ...
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/non_pred3_2.mat',
    ...
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/non_fp3_2.mat',
    ...
    plt.savefig('fig/non_pred_3_2.png')
#     plt.savefig('fig/non_pred_3_2.pdf')
    plt.show()
    ...
    ax.zaxis.set_major_formatter(plt.FuncFormatter(lambda val, pos: '{:.1f}'.format(val)))
    ...
    ax.yaxis.labelpad = 20
    ax.zaxis.labelpad = 20
    plt.savefig('fig/non_real_3_2.png')
#     plt.savefig('fig/non_real_3_2.pdf')
    plt.show()

    save_path = 'model/non_3_2.pth'
