...
#     model_path = torch.load('model/lcr1_10gy.pth')
    # 创建随机训练数据集
    num_samples = 1000
...
#     model.train()
    ...
    data = scipy.io.loadmat('data/lcr_pred10.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/lcr_fp10.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    for epoch in range(num_epochs):
        optimizer.zero_grad()
        loss, pde_loss, ic_loss, bc_loss = compute_loss(model, x_train, t_train, x_data, t_data, real, x_fp, t_fp,  previous_solution)
        loss.backward()
...
#         fp_losses.append(loss_fp.item())
    ...
    def closure():
        ...
        loss, pde_loss, ic_loss, bc_loss = compute_loss(model, x_train, t_train, x_data, t_data,
...
#         lbfgs_loss_fp.append(loss_fp.item())
        print(
            f"Iteration: {iteration} | Total Loss: {loss.item()} | PDE loss: {pde_loss.item()} | IC loss: {ic_loss.item()} | BC loss: {bc_loss.item()} ")
        loss.backward()
...
#     plt.plot(FP_losses, label='FP Loss')
    ...
    plt.legend()
    plt.savefig('fig/loss0.1_5p.png')
#     plt.savefig('fig/loss10_20.pdf')
    ...
    rho = 1
    h_x = 10*torch.exp(-(X - torch.pi) ** 2 / (0.2*torch.pi ** 2))
...
#     t = t.unsqueeze(1)
#     solution = model(x, t)
#     scipy.io.savemat('data/lcr_pred10_20.mat',
...
#     u_fp = model(x, t_fp)
#     scipy.io.savemat('data/lcr_fp10_20.mat',
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_pred_10_5p.png')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_real_10_5p.png')
