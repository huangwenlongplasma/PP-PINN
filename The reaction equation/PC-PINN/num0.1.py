...
#     model_path = torch.load('model/lcr0.1_4.pth')
    # 创建随机训练数据集
    num_samples = 1000
...
#     model.train()
    ...
    data = scipy.io.loadmat('data/lcr_pred10_4.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/lcr_fp10_4.mat')
    x_fp = data['x']
    t_fp = data['t']
...
#     plt.plot(FP_losses, label='FP Loss')
    ...
    plt.legend()
    plt.savefig('fig/loss0.1_10p.png')
...
#     保存数据
    ...
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/lcr_pred10_5.mat',
    ...
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/lcr_fp10_5.mat',
...
#     ax.set_zmargin(0.2)  # 增加 z 轴的边距
    plt.savefig('fig/lcr_pred_0.1_5p.png')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_real_0.1_5p.png')
