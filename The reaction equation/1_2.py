...
def compute_loss(model, x, t, x_data, t_data, real, x_fp, t_fp,  previous_solution, rho=1):
    ...
    loss_ic = torch.mean((u0 - torch.exp(-(x - torch.pi)**2 / torch.pi**2)) ** 2)
...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/lcr1_1tanh1.pth')
    # 创建随机训练数据集
    num_samples = 1000
    x_train = (2 * np.pi * torch.rand(num_samples, 1)).to(device)
    x_train.requires_grad = True
    t_train = (20 * torch.rand(num_samples, 1)).to(device)
    ...
    data = scipy.io.loadmat('data/lcr_pred1_1tanh1.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/lcr_fp1_1tanh1.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    optimizer_lbfgs = torch.optim.LBFGS(model.parameters(),  max_iter=2000, max_eval=None, tolerance_grad=0,
    ...
    plt.legend()
    plt.savefig('fig/loss1_20tanh1.png')
    plt.savefig('fig/loss1_20tanh1.pdf')
    plt.show()
    # 测试模型
    model.eval()
    ...
    rho = 1
    h_x = torch.exp(-(X - torch.pi) ** 2 / torch.pi ** 2)
    ...
    plt.xlabel('t')
    plt.ylabel('x')
    def scale_x(value, index):
        return '{:.2f}'.format(value / 20)

    # 设置 x 轴的主要刻度格式化器
    plt.gca().xaxis.set_major_formatter(FuncFormatter(scale_x))
    plt.savefig('fig/lcr_error_1_20tanh1.png')
    plt.savefig('fig/lcr_error_1_20tanh1.pdf')
    plt.show()
    ...
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/lcr_pred1_20tanh1.mat',
                     {'x': x.cpu().detach().numpy().reshape(-1, 1), 't': t.cpu().detach().numpy().reshape(-1, 1),
                      'u': solution.cpu().detach().numpy().reshape(-1, 1)})
    t_fp = torch.ones_like(x) * 20
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/lcr_fp1_20tanh1.mat',
...
#     ax.set_zticklabels([f'{z:.1f}' for z in z_ticks])
    ax.plot_surface(X.cpu().detach().numpy().reshape(1000, 1000), T_scaled.cpu().detach().numpy().reshape(1000, 1000),uxt.reshape(1000, 1000), cmap='jet')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    ax.set_zticks([0.4, 0.7, 1.0])
    for t in ax.zaxis.get_major_ticks():
        t.set_pad(10)

    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_pred_1_20tanh1.png')
#     plt.savefig('fig/lcr_pred_1_20tanh1.pdf')
#     plt.savefig('fig/fm1pp-pinn.png')
#     plt.savefig('fig/fm1pp-pinn.pdf')
    plt.show()

    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
#     ax.set_zticks([real_u.min(), (real_u.max() + real_u.min()) / 2, real_u.max()]) # z 轴分为三个刻度
#     z_ticks = [real_u.min(), (real_u.max() + real_u.min()) / 2, real_u.max()]
#     ax.set_zticklabels([f'{z:.1f}' for z in z_ticks])
    ax.plot_surface(X.cpu().detach().numpy().reshape(1000, 1000), T_scaled.cpu().detach().numpy().reshape(1000, 1000),real_u.reshape(1000, 1000), cmap='jet')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_real_1_20tanh1.png')
#     plt.savefig('fig/lcr_real_1_20tanh1.pdf')
    plt.savefig('fig/fm1exact.png')
#     plt.savefig('fig/fm1exact.pdf')
    plt.show()

    save_path = 'model/lcr1_20tanh1.pth'
