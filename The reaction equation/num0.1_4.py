...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/lcr0.1_3.pth')
    # 创建随机训练数据集
    num_samples = 1000
    x_train = (2 * np.pi * torch.rand(num_samples, 1)).to(device)
    x_train.requires_grad = True
    t_train = (15 * torch.rand(num_samples, 1)).to(device)
    ...
    data = scipy.io.loadmat('data/lcr_pred10_3.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/lcr_fp10_3.mat')
    x_fp = data['x']
    t_fp = data['t']
...
#     ax.set_zmargin(0.2)  # 增加 z 轴的边距
    plt.savefig('fig/lcr_pred_0.1_4.png')
...
#     ax.set_zlabel('u', fontsize=16)
    ...
    # 自动调整子图参数
    plt.tight_layout()
    plt.savefig('fig/lcr_real_0.1_4.png')
#     plt.savefig('fig/lcr_real_10_20.pdf')
    plt.show()

    save_path = 'model/lcr0.1_4.pth'
