...
# 定义损失函数
def compute_loss(model, x, t, x_data, t_data, real, x_fp, t_fp,  previous_solution, beta=20):
    u = model(x, t)
...
if __name__ == '__main__':
    ...
    model_path = torch.load('model/lcctanh20_7.pth')
    # 创建随机训练数据集
    num_samples = 1000
    x_train = (2 * np.pi * torch.rand(num_samples, 1)).to(device)
    x_train.requires_grad = True
    t_train = (8 * torch.rand(num_samples, 1)).to(device)
    ...
    data = scipy.io.loadmat('data/lcctanh20_pred7.mat')
    real = data['u']
    ...
    data = scipy.io.loadmat('data/lcctanh20_fp7.mat')
    x_fp = data['x']
    t_fp = data['t']
    ...
    plt.legend()
    plt.savefig('fig/losstanh20_8.png')
#     plt.savefig('fig/losstanh20_8.pdf')
    plt.show()
    # 测试模型
    model.eval()
    x = torch.linspace(0, 2 * torch.pi, 1000)
    t = torch.linspace(0, 8, 1000)
    ...
    t = torch.linspace(0, 8, 1000).to(device)
    x = x.unsqueeze(1)
    t = t.unsqueeze(1)
    solution = model(x, t)
    scipy.io.savemat('data/lcctanh20_pred8.mat',
                     {'x': x.cpu().detach().numpy().reshape(-1, 1), 't': t.cpu().detach().numpy().reshape(-1, 1),
                      'u': solution.cpu().detach().numpy().reshape(-1, 1)})
    t_fp = torch.ones_like(x) * 8
    u_fp = model(x, t_fp)
    scipy.io.savemat('data/lcctanh20_fp8.mat',
    ...
    # plt.ylim(0, 2*np.pi)
    plt.title('pred')
    plt.savefig('fig/lcctanh_pred_8.png')
#     plt.savefig('fig/lcctanh_pred_8.pdf')
    plt.show()
    ...
    plt.ylabel('x')
    plt.title('real')
    plt.savefig('fig/lcctanh_real_8.png')
#     plt.savefig('fig/lcctanh_real_8.pdf')
    plt.show()

    save_path = 'model/lcctanh20_8.pth'
